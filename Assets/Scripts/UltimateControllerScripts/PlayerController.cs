    ...
    public class PlayerController : MonoBehaviour, IPlayerController {
        ...
        private void RunCollisionChecks() {
            ...
            bool RunDetection(RayRange range) {
                foreach (var layer in _groundLayer)
                {
                    if (EvaluateRayPositions(range).Any(point => Physics2D.Raycast(point, range.Dir, _detectionRayLength, layer)))
                        return true;
                }
        ...
        private void MoveCharacter() {
            ...
            var hit = OverlapBoxForLayerArray(furthestPoint, _characterBounds.size, 0, _groundLayer);
            if (!hit) {
            ...
            for (int i = 1; i < _freeColliderIterations; i++) {
                ...
                if (OverlapBoxForLayerArray(posToTry, _characterBounds.size, 0, _groundLayer)) {
        ...
        private Collider2D OverlapBoxForLayerArray(Vector2 point, Vector2 size, float angle, LayerMask[] _groundLayers)
        {
            Collider2D collider = null;
            foreach (var layer in _groundLayers)
            {
                collider = Physics2D.OverlapBox(point, size, angle, layer);
                if (collider != null)
                {
                    return collider;
                }
